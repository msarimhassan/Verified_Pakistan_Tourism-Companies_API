...
//get company by parameter

app.get('/getcompany/:name/:dts',async(req,res)=>{
  console.log(req.params.name)
  console.log(req.params.dts)
  const companiesname=[];
const DTSnumber=[];
const email=[];
const verifiedcomp=[]
...
// $('td[data-x="9"]',html).each(function(){ email.push($(this).text());})
 ...
 for(let i=0;i<DTSnumber.length;i++)
 {
    ...
    //Find desired Objects
      
    let companyName=req.params.name;
    let dtsNumber=req.params.dts;

verifiedcomp.forEach(company=>{
        if(company.DtsNumber.trim().toLowerCase()==dtsNumber.toLowerCase() && company.CompanyName.trim().toLowerCase()==companyName.toLowerCase())
        {
